...
class HelpExchangeService {
  ...
  String get _userId =>
      ...
      (throw StateError(
        'You must be signed in to use Sisonke.',
      ));
  ...
  Future<String> createRequest({
    ...
    final post = await _client
        ...
        .insert({
          ...
          // IMPORTANT:
          // The Supabase post_type enum uses
          // "help_needed", not "help_request".
          'type': 'help_needed',
  ...
  Future<void> offerHelp({
    ...
    await _client.from('help_offers').insert({
  ...
  Stream<List<HelpOffer>> watchOffers(
    String requestId,
  ) =>
      _client
          .from('help_offers')
  ...
  Future<List<HelpOffer>> fetchOffers(
    String requestId,
  ) async {
    final rows = await _client
        .from('help_offers')
        .select(
          '*, helper:profiles!help_offers_helper_id_fkey(first_name)',
        )
        .eq(
          'help_request_id',
          requestId,
